...
const SendMail = () => {
  ...
  const handleBulkSend = async () => {
    if (selectedEmails.length === 0)
      return toast.error("Select subscribers first! 🎯");
    const mailToast = toast.loading(
  ...
  return (
    <div className="min-h-screen bg-base-100 p-4 md:p-8 mt-12 relative pb-32">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-base-200 p-8 rounded-[2.5rem] border border-base-content/5 shadow-xl flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <button
              onClick={() =>
                setSelectedEmails(filteredEmails.map((e) => e.mail))
              }
              className="btn btn-ghost rounded-xl gap-2 text-primary font-bold"
            >
              ...
              <table className="table w-full border-separate border-spacing-y-3">
                ...
                <tbody>
                  {currentEmails.map((email) => (
                    <tr
                      key={email._id}
                      className={`group bg-base-100 shadow-sm rounded-2xl transition-all hover:translate-x-1 ${selectedEmails.includes(email.mail) ? "ring-2 ring-primary bg-primary/5" : ""}`}
                    >
                        ...
                        <input
                          ...
                          onChange={() =>
                            setSelectedEmails((prev) =>
                              prev.includes(email.mail)
                                ? prev.filter((e) => e !== email.mail)
                                : [...prev, email.mail],
                            )
                          }
                        />
            ...
            <div className="flex justify-between items-center mt-10 px-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="btn btn-sm rounded-xl bg-base-100 shadow-md"
                >
                  Back
                </button>
                <button
      ...
      <div
        className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 transform ${selectedEmails.length > 0 ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}
      >
        <div className="bg-neutral text-neutral-content px-8 py-4 rounded-[2rem] shadow-2xl flex items-center gap-8 backdrop-blur-xl bg-opacity-90 border border-white/10">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="btn btn-ghost btn-sm rounded-xl hover:bg-white/5 uppercase text-[10px] font-black tracking-widest"
            >
              Cancel
            </button>
            <button
